...
import { signInAction, signUpAction, signInWithGoogleAction, resetPasswordAction, type AuthState } from "@/app/(auth)/actions";
        ...
        <>
          ...
          <form action={async () => { await signInWithGoogleAction(initialState, new FormData()); }}>
            <button
              type="submit"
              disabled={pending}
              className="h-12 w-full rounded-xl border border-slate-200 bg-white text-sm font-bold text-slate-700 transition hover:bg-slate-50 disabled:opacity-60 flex items-center justify-center gap-2"
            >
