...
import {
  ...
  getProductStockMovementsAction,
  addStockLotAction,
...
export function InventorySection({ productId, productName, suppliers, currency, canWrite, compact = false }: Props) {
  ...
  function applyMovementRange(event: FormEvent<HTMLFormElement>) {
    ...
    startMovementTransition(async () => {
      try {
        const result = await getProductStockMovementsAction(productId, {
          from: movementFrom,
          to: movementTo,
        });
        ...
        console.error(error);
        setMovementError("We couldn't load movement history. Please try again.");
        setMovementErrorCode(null);
      }
    });
  }
  ...
  const movementHasRange = Boolean(
    appliedMovementFilters.from || appliedMovementFilters.to,
  );
            ...
            {loading ? (
              ...
              <div>
                ...
                {activeSubTab === "movements" && (
                  <div>
                    <form
                      onSubmit={applyMovementRange}
                      aria-busy={movementPending}
                      className="mb-3 rounded-lg border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900"
                    >
                      <div className="grid gap-3 min-[380px]:grid-cols-2 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto_auto] md:items-end">
                        <label className="block min-w-0">
                          ...
                          <input
                            ...
                            aria-invalid={movementErrorCode === "invalid_from" || movementErrorCode === "reversed"}
                            className="mt-1 h-9 w-full rounded-md border border-slate-200 bg-white px-2 text-sm outline-none focus:border-blue-600 dark:border-slate-700 dark:bg-slate-950"
                          />
                        </label>
                        <label className="block min-w-0">
                          <span className="text-[10px] font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">
                          ...
                          <input
                            ...
                            aria-invalid={movementErrorCode === "invalid_to" || movementErrorCode === "reversed"}
                            className="mt-1 h-9 w-full rounded-md border border-slate-200 bg-white px-2 text-sm outline-none focus:border-blue-600 dark:border-slate-700 dark:bg-slate-950"
                          />
                        </label>
                        <button
                          ...
                          className="inline-flex h-9 items-center justify-center gap-1.5 rounded-md bg-slate-900 px-3 text-xs font-bold text-white disabled:opacity-60 dark:bg-slate-100 dark:text-slate-900"
                        >
                          {movementPending && <Loader2 className="size-3.5 animate-spin" />}
                          {movementPending ? "Applying…" : "Apply"}
                        </button>
                        <button
                          type="button"
                          onClick={resetMovementRange}
                          disabled={movementPending}
                          className="h-9 rounded-md border border-slate-200 px-3 text-xs font-bold text-slate-700 hover:bg-white disabled:opacity-60 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
                        >
                          Reset
                        </button>
                      </div>
                      {movementError && (
                        <p
                          role="alert"
                          className="mt-3 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold text-red-700 dark:border-red-900 dark:bg-red-950/30 dark:text-red-300"
                        >
                          {movementError}
                        </p>
                      )}
                    ...
                    {movements.length === 0 && !movementError ? (
                      ...
                      !movementError && <div className="overflow-x-auto">
                        <table className="w-full text-left text-xs min-w-[500px]">
                          <thead>
                            <tr className="border-b border-slate-200 text-[10px] font-bold uppercase tracking-wider text-slate-400 bg-slate-50">
                          ...
                          <tbody>
                            {movements.map((m) => (
                              <tr key={m.id} className="border-b border-slate-100 hover:bg-[#fff]/50">
                                <td className="px-2.5 py-2 text-slate-500 whitespace-nowrap">{fmtMovementDate(m.created_at)}</td>
                                <td className="px-2.5 py-2">
