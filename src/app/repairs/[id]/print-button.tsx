...
export function PrintButton({ whatsappHref }: PrintButtonProps) {
  ...
  const printThermal = useCallback(async () => {
    ...
    try {
      ...
      if (!receipt) throw new Error("missing thermal repair receipt");

      receipt.dataset.repairsThermalMeasuring = "true";
      ...
      if (Math.abs(measuredWidthMm - THERMAL_CONTENT_WIDTH_MM) > 0.5) {
        throw new Error("invalid thermal repair receipt width");
      }
      ...
      if (
        ...
        throw new Error("invalid thermal repair receipt height");
      }

      delete receipt.dataset.repairsThermalMeasuring;
      if (!isAttemptActive(attempt)) return;
      const style = document.createElement("style");
      style.id = THERMAL_PAGE_STYLE_ID;
      style.textContent = `@page repairsThermalReceipt { size: ${THERMAL_PAGE_WIDTH_MM}mm ${pageHeightMm.toFixed(1)}mm; margin: 4mm; }`;
      if (!isAttemptActive(attempt)) return;
      document.head.append(style);
      if (!isAttemptActive(attempt)) return;
      document.body.dataset.printMode = "thermal";
      document.body.dataset.repairsThermalPrint = "true";
      await nextAnimationFrame();
      if (!isAttemptActive(attempt)) return;
