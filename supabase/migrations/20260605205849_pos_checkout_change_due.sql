-- Migration: Record POS change due without overstating received money
--
-- p_amount_paid keeps the existing RPC signature for deploy safety, but the
-- function now treats it as the amount tendered at checkout. Only the settled
-- amount is recorded as received money; any overage is stored as change_due.

alter table public.invoices
  add column if not exists amount_tendered numeric(12,2) not null default 0 check (amount_tendered >= 0),
  add column if not exists change_due numeric(12,2) not null default 0 check (change_due >= 0);

comment on column public.invoices.amount_tendered is 'Amount entered at POS checkout before any returned change.';
comment on column public.invoices.change_due is 'Amount returned to the customer as change; not recorded as received payment.';
...
begin
  ...
  insert into public.invoices (
    ...
    amount_tendered, change_due, note, created_by
  ) values (
    ...
    v_amount_tendered, v_change_due, nullif(p_note, ''), v_profile_id
  );
