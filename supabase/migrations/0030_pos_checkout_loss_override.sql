-- Migration 0030: Add per-checkout loss override parameter to pos_checkout
--
-- Adds a p_allow_loss_override boolean parameter (default false) to the
-- pos_checkout function. When true, the below-cost sale block is bypassed
-- for that specific checkout call, WITHOUT mutating products.allow_sell_at_loss.
--
-- The existing loss-prevention audit trail remains intact: every below-cost
-- sale is still recorded to audit_logs (action pos.loss_sale_completed) and
-- copied to loss_prevention_events by the existing trigger.
--
-- Backward-compatible: p_allow_loss_override defaults to false, so existing
-- callers (including the original signature) behave identically.
