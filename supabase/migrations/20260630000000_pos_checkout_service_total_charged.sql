-- Migration: fix service sale checkout to use service_total_charged as the line total
--
-- Bug: service metadata (principal/commission/total charged) was stored, but the
-- invoice line total used products.sale_price (zero for seeded services), so the
-- invoice grand total, paid amount, and payment rows were all zero.
--
-- Fix: inside pos_checkout, when processing service lines, derive the effective
-- unit price from service_total_charged (falling back to principal + commission).
-- The cart unit_price is still honored when present so existing/future clients
-- that send a computed value continue to work. Physical products are unchanged.
--
-- No signature, schema, RLS, permission, or invoice numbering changes.
-- No backfills. Historical invoices are intentionally left untouched.
...
revoke all on function public.pos_checkout(uuid, uuid, jsonb, numeric, public.payment_method, numeric, text, text, boolean, text) from public;
...
comment on function public.pos_checkout(uuid, uuid, jsonb, numeric, public.payment_method, numeric, text, text, boolean, text) is
  'Processes POS checkout. Service lines now use service_total_charged (or principal+commission) as the effective unit price when the cart does not provide one.';
