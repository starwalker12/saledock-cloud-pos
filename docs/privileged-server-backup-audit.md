...
| Auth password/session actions | First-time password provider update; current user's auth sessions | Authenticated user; `updateUserById(user.id)` and session query by `user.id` | Returns only safe state/session fields | One raw Admin error is masked in draft PR #269. |
...
| Staff permission editor | Write permission overrides despite RLS | Owner/Admin action; organization ID from current profile | Returns editor-safe fields only | Crafted target membership check is added in draft PR #269. |
...
| Import status permission | Gap found | Draft PR #270 adds the missing Owner/Admin guard. |
...
| File type/size | UI advertised ZIP under 50 MB but did not enforce it | Draft PR #270 enforces ZIP extension, non-empty file, and 50 MB maximum before parsing. |
...
| User-facing errors | Top-level errors use `getSafeActionError`; per-row warnings leaked raw DB text | Draft PR #270 replaces raw warning text with safe record-specific wording. |
...
- The browser parses the full ZIP/SQLite payload in memory. The 50 MB cap in
  draft PR #270 limits, but does not eliminate, low-memory mobile risk.
...
- [ ] Verify non-ZIP, empty, and oversized files are rejected after PR #270.
...
production mutation. Drafts #269 and #270 change no money, stock, FIFO, payment,
