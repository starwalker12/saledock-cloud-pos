import { expect, test, type Page, type BrowserContext, type Dialog } from "@playwright/test";
import type { ConsoleMessage } from "@playwright/test";

const env = process.env.CSP_TEST_ENV as "dev" | "local-production" | "preview" | "production" | undefined;
const baseUrl = process.env.PLAYWRIGHT_BASE_URL || "http://localhost:3000";

const routes = ["/", "/login", "/privacy", "/terms", "/auth/invite?error=otp_expired"];
const viewports = [
  { width: 390, height: 844 },
  { width: 1440, height: 900 },
];

test.skip(!env || !["dev", "local-production", "preview", "production"].includes(env), "CSP_TEST_ENV must be set");
...
async function dismissAnalyticsBanner(page: Page): Promise<void> {
  await page.evaluate(() => {
    try {
      const rejected = { value: "rejected", version: "qa-test", timestamp: new Date().toISOString() };
...
function hydrateWarnings(messages: ConsoleMessage[]): string[] {
  return messages
    ...
    .filter((text) => /hydration|hydrated|server rendered HTML|did not match|mismatch|nonce/i.test(text));
}

async function verifyNonceUniqueness(context: BrowserContext, route: string): Promise<{ unique: boolean }> {
  const res1 = await context.request.get(`${baseUrl}${route}`, { maxRedirects: 5 });
  const res2 = await context.request.get(`${baseUrl}${route}`, { maxRedirects: 5 });
...
async function inspectDomScripts(page: Page, responseNonce: string | null): Promise<{
  colorTheme: { present: boolean; nonceNonEmpty: boolean; attrNonceMatches: boolean; propNonceMatches: boolean };
  nextRoots: { total: number; withNonce: number; withoutNonce: number; mismatch: number };
  externalRuntime: { total: number; withNonce: number; withoutNonce: number; mismatch: number };
  inlineOther: { total: number; withNonce: number; withoutNonce: number; mismatch: number };
  jsonLd: { total: number; withNonce: number; withoutNonce: number; mismatch: number };
}> {
  return page.evaluate((opts) => {
    ...
    const colorTheme = scripts.find((s) => {
      ...
      return text.includes("data-color-theme") || text.includes("DEFAULT_COLOR_THEME") || text.includes("colorThemeInitScript");
    });
    ...
    const jsonLd = scripts.filter((s) => s.getAttribute("type") === "application/ld+json");
    ...
    function check(script: HTMLScriptElement | undefined) {
      if (!script) return { nonceNonEmpty: false, attrNonceMatches: false, propNonceMatches: false };
    ...
    function summarize(list: HTMLScriptElement[]) {
      ...
      const mismatch = checks.filter((c) => c.nonceNonEmpty && !c.propNonceMatches && !c.attrNonceMatches).length;
...
for (const route of routes) {
  for (const viewport of viewports) {
    test(`${env} ${route} ${viewport.width}x${viewport.height}`, async ({ page, context }) => {
      ...
      if (isReadOnlyEnv) {
        await page.route(`${baseUrl}/api/csp-report`, async (route) => {
          await route.abort();
        });
      }
      ...
      const headers = {
        ...
        reportingEndpoints: Boolean(response?.headers()["reporting-endpoints"]),
      ...
      test.info().annotations.push({ type: "external-runtime-scripts", description: `${domScripts.externalRuntime.withNonce}/${domScripts.externalRuntime.total} with nonce, ${domScripts.externalRuntime.withoutNonce} without nonce, ${domScripts.externalRuntime.mismatch} mismatch` });

      if (domScripts.jsonLd.total > 0) {
        expect(domScripts.jsonLd.mismatch).toBe(0);
      }

      test.info().annotations.push({ type: "external-runtime-scripts", description: `${domScripts.externalRuntime.withNonce}/${domScripts.externalRuntime.total} with nonce, ${domScripts.externalRuntime.mismatch} mismatch` });
      test.info().annotations.push({ type: "inline-other-scripts", description: `${domScripts.inlineOther.withNonce}/${domScripts.inlineOther.total} with nonce, ${domScripts.inlineOther.mismatch} mismatch` });
      test.info().annotations.push({ type: "json-ld-scripts", description: `${domScripts.jsonLd.withNonce}/${domScripts.jsonLd.total} with nonce, ${domScripts.jsonLd.mismatch} mismatch` });
      ...
      const nonTrivialRequestFailures = requestFailures.filter((f) => {
        ...
        if (f.includes("/_vercel/insights/") || f.includes("/_vercel/speed-insights/")) return false;
      ...
      const hasHydrationWarning = redactedWarnings.some((w) => /hydration|hydrated|server rendered HTML/i.test(w));
      const hasNonceMismatch = redactedWarnings.some((w) => /nonce|did not match|mismatch/i.test(w));

      test.info().annotations.push({ type: "hydration-warning", description: hasHydrationWarning ? "observed" : "none" });
      test.info().annotations.push({ type: "nonce-mismatch", description: hasNonceMismatch ? "observed" : "none" });
      test.info().annotations.push({ type: "native-dialogs", description: String(dialogs.length) });
      test.info().annotations.push({ type: "page-errors", description: String(pageErrors.length) });
      test.info().annotations.push({ type: "framework-overlay", description: "none" });
    });
  }
}
